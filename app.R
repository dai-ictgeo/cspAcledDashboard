...
ui <- navbarPage("Conflict Events Report", theme = shinytheme("paper"),
                           tabPanel("Overview",
                                    fluidPage(
                                    h4("Country Overview"),
                                    fluidRow(
...
server <- function(input, output) {
  ...
  output$overviewMap <- renderLeaflet({
    leaflet(data = ACLED_Data) %>% addTiles(urlTemplate = tcu_map, attribution = map_attr)  %>% addCircleMarkers(~longitude, ~latitude, radius = sqrt(ACLED_Data$fatalities))})
  
  #Events Over Time
  Monthly <- ACLED_Data %>% group_by(month=floor_date(event_date, "month")) %>% summarise(total = length(month))
  
  monthlyEvents <- ggplot(Monthly, aes(month, total)) + geom_line() + ggtitle("Total events per month over time") + theme_classic()
  
  output$eventsPerMonth <- renderPlotly({monthlyEvents})
  ...
  eventsYear <-  ggplot(ACLED_Data, aes(year, fill=admin1)) + geom_bar() + ggtitle("Events Per Year") + theme(plot.title = element_text(size=14, face="bold")) + theme_classic()
  
   output$eventsPerYear <- renderPlotly({ggplotly(eventsYear)})
   
   fatalitiesYear <- ggplot(ACLED_Data, aes(x=year, y=fatalities, fill=admin1)) + geom_bar(stat="identity") + ggtitle("Fatalities Per Year") + theme(plot.title = element_text(size=14, face="bold")) + theme_classic()
